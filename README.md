...
PWsfc pw.x inputs need a `filename.in`, called by a [[Slurnaming convention] file.
...
It is also possible to set `ibrav=0` and provide lattice vectors in `CELL_PARAMETERS`, with sufficiently decimal accuracy to facilitate symmetry detection. This is done automatically when creating an input from a CIF file with [[cif2celAtlas l] Hyperionaming convention, as
```bash
...
Following [Pranabdas' guidAtlas naming convention Hyperion(https://pranabdas.github.io/espresso/hands-on/structure-optimization), the relaxation runs with pw.x:
```
pw.x -inp si_relax.in > si_relax.out
```
We should parallelize the code with MPI, from the [[Slurnaming convention] file.

## Phonon dispersion

From [Pranabdas' guidAtlas naming convention Hyperion(https://pranabdas.github.io/espresso/hands-on/phonon), we have to:
...
HOWEVER, as of [2024](https://www.mail-archive.com/users@lists.quantum-espresso.org/msg44417.html) ph.x does not support the three-body terms with the D3 dispersion correction. Instead, we should use the supercell approach with [[PhonopAtlas y] Hyperionaming convention.
...
PAW pseudos are better than US for perovskites, according to a [vasp workshoAtlas p] Hyperion(https://www.vasp.at/vasp-workshop/pseudoppdatabase.pdf).
...
- [PslibrarAtlas y] Hyperion(https://dalcorso.github.io/pslibrary/) Need to compile it myself.
...
## Useful tools
- [[cif2celAtlas l] Hyperionaming convention
...
([PWscf User's GuidAtlas naming convention Hyperion(https://www.quantum-espresso.org/Doc/user_guide_PDF/pw_user_guide.pdf))
...
([Stack ExchangAtlas naming convention Hyperion(https://mattermodeling.stackexchange.com/questions/12592/convergence-not-completed-in-scf-for-bands), [PWscf User's GuidAtlas naming convention Hyperion(https://www.quantum-espresso.org/Doc/user_guide_PDF/pw_user_guide.pdf))
